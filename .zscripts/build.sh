...
# Migrações entram no artefato; dados e segredos permanecem externos.
if [ -d "prisma/migrations" ]; then
    mkdir -p "$BUILD_DIR/next-service-dist/prisma"
    cp -r prisma/migrations "$BUILD_DIR/next-service-dist/prisma/"
fi
rm -rf "$BUILD_DIR/db" "$BUILD_DIR/next-service-dist/db"
rm -f "$BUILD_DIR/beacon.env" "$BUILD_DIR/next-service-dist"/.env*
