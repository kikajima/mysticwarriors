...
# O volume deve estar montado e restaurado antes do primeiro início.
case "${DATABASE_URL:-}" in
    file:/*) ;;
    *) echo "ERRO: configure DATABASE_URL=file:/volume-persistente/custom.db" >&2; exit 1 ;;
esac
DB_FILE="${DATABASE_URL#file:}"
DB_FILE="${DB_FILE%%\?*}"
if [ ! -f "$DB_FILE" ]; then
    echo "ERRO: banco configurado ausente; restaure o backup no volume antes de iniciar." >&2
    exit 1
fi
unset GM_SEED_DB

# migrações dentro do pacote standalone (migrador de boot do app)
if [ -d "$BUILD_DIR/next-service-dist/prisma/migrations" ]; then
    export GM_MIGRATIONS_DIR="$BUILD_DIR/next-service-dist/prisma/migrations"
fi
...
if [ -f "./next-service-dist/server.js" ]; then
    ...
    echo "🗄️  DATABASE_URL: ${DATABASE_URL:-<padrão do pacote>}"
