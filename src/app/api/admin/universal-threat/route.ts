...
export async function POST(request: Request) {
  try {
    ...
    const rl = rateLimit(`admin-universal-threat:${clientIp(request)}`, 10, 5 * 60_000);
    if (!rl.allowed) {
      throw new ApiError('RATE_LIMITED', 'Muitas invocações administrativas. Aguarde alguns minutos.');
    }
