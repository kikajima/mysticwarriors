...
export function GuildsPanel({ player, onAction, busy }: { player: PlayerView; onAction: (payload: Record<string, unknown>) => Promise<boolean>; busy: boolean }) {
  ...
  const roster = (g: GuildDetail, manage: boolean) => <ul className="space-y-3">{g.members.map(m => <li key={m.id} className="rounded border border-amber-900/50 p-3 space-y-2">
    ...
    <p className="text-sm">⚡ {m.power.toLocaleString('pt-BR')} · {m.online ? 'Online' : 'Offline'}{manage ? ` · Doou ${m.donated ?? 0} Créditos` : ''}</p>
  ...
  return <div className="space-y-5 text-amber-100">
    ...
    {myGuild ? <GameCard className="p-4 space-y-4">
      ...
      <p>{myGuild.members.length}/{myGuild.capacity} membros · Total doado: {myGuild.totalDonated.toLocaleString('pt-BR')} Créditos</p>
      ...
      {myGuild.level < 10 ? <div className="space-y-2"><label htmlFor="guild-donation">Progresso para Nv {myGuild.level + 1}: {levelProgress.toLocaleString('pt-BR')} / {levelCost.toLocaleString('pt-BR')} Créditos</label><progress className="w-full" value={levelProgress} max={levelCost} /><input id="guild-donation" className={input} type="number" min="1" step="1" value={donation} onChange={e => setDonation(e.target.value)} /><GameButton disabled={busy} onClick={() => void run({ type: 'donate_guild', amount: Number(donation) })}>Doar Créditos</GameButton></div> : <p>Nível máximo alcançado</p>}
      ...
      <h3 className="font-heading">Maiores doadores</h3><ol>{myGuild.donors?.map(d => <li className="break-words" key={d.id}>{d.name}: {d.total.toLocaleString('pt-BR')} Créditos</li>)}</ol>
    ...
    </GameCard> : <GameCard className="p-4 space-y-3"><label htmlFor="guild-name">Nome da nova guilda</label><input id="guild-name" className={input} maxLength={24} value={name} onChange={e => setName(e.target.value)} /><GameButton disabled={busy || player.zeni < 5000} onClick={() => void run({ type: 'create_guild', guildName: name })}>Fundar por 5.000 Créditos</GameButton></GameCard>}
