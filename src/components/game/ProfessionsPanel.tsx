        ...
        <div className="flex flex-col md:flex-row md:items-center gap-5">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-amber-200/60 mt-1 leading-relaxed">
              Escolha quanto tempo seu radar ficará procurando. A busca não gasta energia. A chance base cresce até 20%; bônus dos dois espaços de acessório podem elevar a chance total até 50%. Cada busca encontra no máximo uma Chave e só pode começar se existir uma estrela sem dono no mundo.
            </p>
            {freeBalls !== undefined && (
              <div className={`mt-3 rounded-lg border p-3 ${
                ...
                <p className="text-[11px] text-amber-200/55 mt-1">
                  {noFreeBalls
                    ? 'As 7 Chaves estão em posse de guerreiros. A busca está indisponível; dispute uma no PvP ou aguarde alguém invocar Aethelgard.'
            ...
            {search && (
              <div className="mt-3 rounded-lg border border-orange-700/50 bg-orange-950/30 p-3">
                ...
                <p className="text-xs text-amber-200/50">A Chave será aplicada apenas quando a busca terminar.</p>
                <GameButton
                  size="sm"
                  variant="ghost"
          ...
          >
            {complete ? 'Conjunto completo' : noFreeBalls ? 'Nenhuma Chave espalhada' : search ? 'Busca em andamento' : `Iniciar busca (${selectedHours}h)`}
          </GameButton>
        </div>
      </GameCard>
      <GameCard className="p-4">
        <p className="text-xs text-amber-200/55 leading-relaxed">
          ⚔️ Uma vitória no PvP também pode roubar 1 Chave do Dragão do adversário. Proteja sua coleção escolhendo bem seus duelos.
        </p>
      </GameCard>
    </div>
  );
