...
test('UI de treino não repete saldo/energia e Transformações exibem descrição + efeitos', async () => {
  ...
  expect(src).toContain('transformationEffectLabels(tr!)');
  expect(src).toContain('Enquanto ativa:');
});
