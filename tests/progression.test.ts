...
describe('Integridade do conteúdo', () => {
  ...
  test('inimigos: três capangas I/II/III por Escala de Poder, com intenção própria', () => {
    ...
    for (let scaleIndex = 0; scaleIndex < POWER_SCALES.length; scaleIndex++) {
      ...
      const powers = group.map(npcCombatPower);
      expect(powers[0]).toBeLessThan(powers[1]);
      expect(powers[1]).toBeLessThan(powers[2]);
