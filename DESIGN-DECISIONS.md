...
- **Amizade exige consentimento:** enviar amizade cria um `ChatFriendRequest` pendente. O destinatário precisa **Aceitar** ou **Recusar** dentro do balão do chat. Somente o aceite cria amizade, gravada em duas linhas espelhadas de `ChatFriend` para que a relação seja recíproca.
- **Pedidos são gerenciáveis:** quem enviou pode cancelar enquanto estiver pendente; quem recebeu vê uma seção própria de **Pedidos de amizade** com Aceitar/Recusar. Convites enviados também ficam visíveis até serem resolvidos.
- **Bloqueio é mais forte que silêncio:** `ChatMute` apenas oculta mensagens para quem silenciou. `ChatBlock` desfaz amizade, cancela convites nos dois sentidos, oculta mensagens do bloqueado para o bloqueador e impede mensagens privadas nos dois sentidos enquanto qualquer um dos dois mantiver o bloqueio.
- **Relações sociais são reversíveis:** remover amigo, cancelar convite, recusar, bloquear e desbloquear são controles do próprio balão do chat. Limpezas continuam funcionando mesmo se a outra personagem tiver sido excluída, evitando relações órfãs impossíveis de limpar.
