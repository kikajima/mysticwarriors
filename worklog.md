# Worklog — Guerreiros Místicos (jogo estilo Bitefight + Dragon Ball)

---
Task ID: 1
Agent: main (Super Z)
Task: Criar jogo web de gerenciamento de recursos "Guerreiros Místicos" (estilo Bitefight, tema Dragon Ball)
...
- Criada lógica do jogo em src/lib/game/:
  ...
  - constants.ts: 5 raças com bônus (Saiyajin/Humano/Namekuseijin/Androide/Majin), 10 missões, 9 inimigos NPC, 21 itens de loja, 15 bots PvP, curva de XP, custos de treino
...
- Missão: Zeni +56, energia -8 ✓
- Treino: custo Zeni/energia aplicado ✓
- Batalha NPC: dialog animado, vitória +90 Zeni, dano persistente ✓
- PvP: derrota → hospital +1 HP, roubo/perda de Zeni, level-up restaura vida ✓
...
- Mecânicas estilo Bitefight: energia regenerável, missões por energia, treino por Zeni, batalhas com HP persistente, hospital, PvP com roubo de Zeni, equipamentos, esferas do dragão + Shenlon
...
Task ID: 2
Agent: main (Super Z)
Task: 6 aprimoramentos do jogo Guerreiros Místicos (dados de nome, contas, guildas, missões temporizadas, mestres/técnicas, equipamentos de treino)
...
- Mestres NPC na aba Treino: 7 mestres (Kame, Mr. Satã, Kuririn, Tenshinhan, Piccolo, Vegeta, Rei Kai) ensinando 11 técnicas de Força ou Ki (Rogafufuken 600z → Genki Dama 80.000z); técnicas aparecem nas batalhas (22% de chance, dano ×2,0-×4,2) com destaque visual 🔥 TÉCNICA no log de combate
...
- Guildas: aba nova no nav; fundar custa 5.000 Zeni; entrar/sair com transferência de liderança e dissolução quando vazia; ranking mostra badge da guilda; dashboard mostra chips de guilda/técnicas/missão
...
- Técnica Rogafufuken aprendida (600z) e usada em batalha: "R1 🔥 TÉCNICA Mestre Akira desata 🐺 Rogafufuken (-57 HP)" ✓
- Guilda "Guerreiros Z" criada por 5.000 Zeni, líder visível, badge no ranking ✓
...
- Novos sistemas: contas (até 3 personagens), guildas (5k Zeni), missões temporizadas (3-60min), técnicas de mestres (Força/Ki) com efeito em combate, equipamentos de treino com bônus de atributo, gerador de nomes com dado
...
- REGRAS CENTRALIZADAS: src/lib/game/rules.ts (STAT_CAP=999 via addStat, Zenkai 2/24h + relevância + anti-repetição, farm PvE 100/50/25/10%, dayKey/weekKey fuso SP)
...
Testes:
- bun test tests/: 52/52 (stat cap, Zenkai, farm, raças=perks, desempate 50/50, físico/Ki, loadout, dados, guildas)
...
- Navegador (agent-browser): convidado→criar→dashboard→batalha (VITÓRIA +64 Zeni)→quests (progresso 1/3)→Salvar guerreiro→refresh restaura sessão; mobile 390px sem overflow, bottom nav ativa, desktop nav 9 seções; VLM aprovou landing/dashboard/batalha/mobile
...
- A6 nomes ORIGINAIS: 60 nomes no gerador (nenhum exato da franquia; sem 'Son' nos títulos; epítetos originais); 15 bots renomeados via scripts/rename-bots.ts (UPDATE preservando IDs/stats) — Kakaroto→Kaoran, Príncipe Vegeta→Príncipe Vejor, Majin Boo→Majin Bumbo etc.
- A7 raças rebalanceadas (nenhum bônus > 15%): Saiyajin 10% físico + 10% XP + Zenkai; Humano 10% defesa + 10% regen energia + treino -10%; Namekuseijin 15% regen vida + 5% Ki + 5% esquiva; Androide 10% velocidade + missões -15% energia + 5% zeni; Majin +5% em tudo + 5% absorção — perks textuais espelham os números
...
- SIM BASELINE (scripts/sim-balance.ts): REPRODUZIU EXATAMENTE os números do usuário — Saiyajin 46,25% | Humano 66,98% | Namekuseijin 36,23% | Androide 14,70% | Majin 85,85% | limite de rodadas 41,7% (10.000 lutas, 500 sementes/par, lados invertidos)
- DIAGNÓSTICO (micro-experimentos scripts/sim-debug-speed.ts):
  ...
  * Majin 1.05×4 eixos compostos → 86%
...
- CALIBRAÇÃO RACIAL (6 iterações com simulação): saiyajin físico 1.08+XP 1.10+Zenkai | humano def 1.07+Ki 1.02+regen 1.10+treino 0.90 | namek Ki 1.05+dodge 0.045+hpRegen 1.15 | androide speed 1.05+dodge 0.02+kiChance 0.02+missões 0.85/1.05 | majin 1.02×4+absorb 0.04
...
- ZENKAI POR RISCO: sem cota diária (window/maxPerWindow removidos); relevância 60%; mesmo adversário 12h; exige entrada com HP≥50%; custo econômico natural via hospital (3 Zeni/HP escala com nível)
...
- PROFISSÕES (substituem as 10 missões temporizadas): 5 (Agricultor 🌾, Cientista 🔬, Acadêmico 🎓, Policial 👮, Atleta 🏃), turno de 1h, custo 6 ⚡ (androide 5); 5 ranks com títulos próprios (Lavrador→Lenda dos Campos etc.); Zeni por turno 300→450→675→1010→1500 (5x gradual, exato); XP = 10%/14%/18%/22%/25% do xpToNextLevel ATUAL (auto-balanceado em qualquer nível); promoção aos 3/4/5/6 turnos (18h por profissão) com bônus único 1k/3k/9k/30k (ledger próprio); chance de esfera 3%→10%; e2e confirmou: 3º turno → "PROMOVIDO a Fazendeiro! +1.000 Zeni" e conta do Zeni fecha exata
...
- e2e-v06.sh NOVO: 27/27 (batalha PvE/PvP debita 3 exatos; INSUFFICIENT_ENERGY; intervalo 300s; turno 1h; androide 5 ⚡; claim antes→MISSION_NOT_READY; rank1=315+progresso 1/3; promoção no 3º turno; conta Zeni exata; excluir único personagem; conta viva com 0; cosméticos comprados+equipados por 💎; xp/xpToNext no estado)
...
- Loja: Treino e Consumíveis custam 💎 diamantes (Elixir do Dragão fixo em 75); Zeni segue para equipamentos de combate
...
- MUDANÇA 1 IMPLEMENTADA: economy.ts grantXp — level-up agora restaura SÓ vida (hp: maxHp; energia nunca; comentário documenta as 3 fontes legítimas de energia: regen 8h, Cápsula da loja, admin restore_energy). page.tsx: 2 toasts de level-up reescritos ("Vida restaurada… energia regenera só com o tempo"). Shenron "vitalidade" e admin restore_energy intocados (não são level-up).
...
Task: Revisão de status + QA; implementar o recurso ESCALA DE PODER (Capítulo 5 das regras ASCENSÃO Z do dono) com selos visuais, progresso e comparação de escala nas cartas de oponentes
...
Stage Summary:
- RECURSO ENTREGUE: Escala de Poder do sistema ASCENSÃO Z integrada à ficha, batalha e ranking — mesma linguagem/nomenclatura do RPG do dono (Cap. 5, regra 5.1), zero mudança no motor de combate (aditivo e puro)
...
- RankingLive.tsx (página pública /ranking): selo compacto de escala (emoji + En) ao lado do poder de cada linha, com tooltip "Escala n — Nome (ASCENSÃO Z)"; funciona tanto na lista da nuvem quanto na reserva do servidor
...
Task ID: v0.9.12
Agent: main (Super Z) — revisão automática (cron 15min)
Task: Revisão de status + QA; CORREÇÃO P0 (jogo injogável no painel de visualização por cookie SameSite) + recurso ARMADURA DE ESCALA (regras 5.1/5.3 do ASCENSÃO Z no motor de combate) + polimento de estilo obrigatório
...
Task ID: v0.9.13
Agent: main (Super Z) — revisão automática (cron 15min)
Task: Revisão de status + QA; RECURSO CENTRAL — ÍMPETO (Cap. 7) e QUEBRA DE LIMITE (Cap. 29) do ASCENSÃO Z integrados ao motor de combate, à UI e ao World Boss + estilização obrigatória

Work Log:
- QA inicial: servidor OK (home/jogar 200), banco pristino (15 bots/0 contas), suíte 276/276, lint/tsc limpos; E2E do fluxo base (guest → criar ImpetoQA → luta vs Saibaman → VITÓRIA +72 Zeni/+45 XP) sem erros de página
- LIDO O LIVRO DO DONO (upload/Pasted Content...txt): Cap. 7 (Ímpeto: máx 6, começa com 1, gatilhos de ganho "uma vez por gatilho por rodada", tabela de custos 1/2/3, "ataques adicionais não geram Ímpeto", Espírito de Superação do Saiyajin) e Cap. 29 (Quebra de Limite: 3 Ímpetos + 3 Ki + 2 Energia, 1×/combate, "+1 Escala" entre os benefícios, duração 2 rodadas)
...
- ENGINE (engine.ts / simulateBattle):
  * estado por combatente: Ímpeto inicial 1; Espírito de Superação SIMÉTRICO (+1 para quem enfrenta oponente ≥1 Escala acima — generalização do talento Saiyajin para as viradas "David vs Golias"); flags halfGained (1×/luta), heavyRound (1×/gatilho/rodada), quebraUsed/quebraRounds
...
- UI/ESTILO (obrigatório):
  ...
  * BattlePanel — NOVO card "Ímpeto — ASCENSÃO Z Cap. 7" (ícone Flame em tile gradiente laranja, resumo das regras com custos coloridos, Espírito de Superação e dica de estratégia); badge "🔥 +1 Ímpeto de superação" nos cards de oponentes ≥1 escala acima (tooltip com a regra)
...
Task: QA de rotina + RECURSO CENTRAL: Talentos de Ímpeto — Cap. 7 do ASCENSÃO Z COMPLETO (os dois gastos restantes de 1 Ímpeto) + estilização obrigatória

Work Log:
- QA inicial: servidor OK (/, /jogar 200), suíte 300/300, banco pristino 15 bots/0 contas; E2E do fluxo base (guest → QARound14 → criar Saiyajin → luta vs Saibaman → VITÓRIA) sem erros de página/console
...
- NOVO src/lib/game/content/talents.ts: TALENTS (Repetição do Destino 🎯 nv4/2400 Zeni; Reposicionamento Dramático 🌀 nv6/3600 Zeni), getTalent, parseTalents (tolerante a lixo), validateTalentPurchase (100% puro — reaproveitado pela action e pelos testes)
...
- E2E agent-browser COMPROVADO: compra na UI (POST buy_talent 200 → card "Dominado" → "Ativo em todas as batalhas"); badges no card de Ímpeto; luta vs Cell Júnior → "[🌀 TalentQA gasta 1 Ímpeto para se reposicionar… mas o instante escapa!]" disparado por golpe pesado real; luta vs Soldado de Freeza → "R1🎯 DESTINOTalentQA acerta um golpe brutal! (-4 HP) [🎯 Repetição do Destino — 1 Ímpeto para repetir o teste!]" — reroll com SUCESSO e dano aplicado; zero erros de console (entrada antiga de erro de sintaxe era cache do navegador — browser novo confirmou limpo)
...
Stage Summary:
- RECURSO ENTREGUE: TALENTOS DE ÍMPETO de ponta a ponta — o Capítulo 7 do ASCENSÃO Z está agora 100% integrado (todos os 5 gastos da tabela: combo 1, defesa heroica 2, quebra de limite 3, repetição de d10 1, reposicionamento 1), com economia de compra, narrativa, visual e testes
...
Task ID: v0.9.16
Agent: main (Super Z) — revisão automática (cron 15min, job 382709)
Task: QA de rotina + RECURSO: Exaustão Pós-Quebra de Limite (Cap. 29 do ASCENSÃO Z — o risco que faltava) + estilização obrigatória
...
- NOVO TALENTO "Segundo Vento" (🌬️, nv8, 5.200 Zeni, custo 2 Ímpetos): engine.ts implementa os TRÊS caminhos de disparo, todos determinísticos (zero rng):
...
- CONQUISTAS NARRATIVAS (categoria nova 'narrative' — 📖 ASCENSÃO Z):
  * ⚡ "Milagre no Limite": vença 3 batalhas nas quais você ativou a Quebra de Limite (metric miracleWins, alvo 3, 6.000 Zeni + 6 cristais)
  * 🐜 "David vs Golias": anule um golpe poderoso com o Reposicionamento Dramático contra oponente 2+ escalas acima (metric davidWins, alvo 1, 4.000 Zeni + 4 cristais) — conta o MOMENTO, mesmo em derrota
...
- UI/ESTILO (obrigatório):
  ...
  * AchievementsPanel: categoria 📖 Narrativa com título em gradiente céu→ciano + selo "ASCENSÃO Z" e cards em gradiente slate→cyan com borda ciano — visual distinto das conquistas "de número"
...
Work Log:
- QA inicial: servidor OK (/, /jogar, /ranking 200), suíte 340/340, banco pristino 0 contas/15 bots; E2E base (guest → QAReview18 → Saiyajin → luta vs Saibaman → VITÓRIA) sem erros de console
- ESCOLHA DE FOCO: a chave de 8 do Grande Torneio — o modo de jogo mais icônico do universo Dragon Ball que faltava, fechando o ciclo PvE (arena) + PvP (ranking) + Torneio (campanha narrativa)
- NOVO src/lib/game/content/tournament.ts (100% puro): elenco FIXO de 8 lutadores com personalidade (quartas: Mestre Kuma 🐻/Tigre de Ferro 🐅/Vera Vento 🌪️; semis: Ronin Blade 🗡️/Irmãs do Gelo ❄️/Sombra Silente 🌙; finais: Grão-Mestre Varnil 👑/Campeã Yurika ⚡) com taunts, epítetos e VIÉS de distribuição de poder (atk/ki/spd/def); rotação de confrontos por runCount (campanhas consecutivas nunca repetem o caminho); TOURNAMENT_ROUNDS (0.82/0.95/1.00, premiação 300/700/1800 Zeni + 12%/18%/30% do XP do nível + 0/1/3 💎); estado da campanha (parse tolerante/startRun/advanceTournament/tournamentCooldownRemainingMs/validateTournamentFight); buildTournamentOpponent — adversário ELÁSTICO construído do combatente REAL do jogador (equipamentos/transformação/talentos contam) × multiplicador da rodada × viés do lutador, SEM técnicas/talentos (a assimetria de recursos é do jogador)
...
- CONQUISTAS (categoria nova 'tournament' — 🏟️ GRANDE TORNEIO, título em gradiente amarelo-âmbar + cards em gradiente amarelo-950): 🏟️ Estreia no Ringue (1 luta, metric tournamentRoundWins), 🏆 Campeão Mundial (1 título, 10.000 Zeni + 10 💎), 👑 Dinastia do Ringue (5 títulos, 45.000 Zeni + 30 💎); achievementMetrics expõe as novas colunas; quest semanal 🏟️ Gladiador da arena (5 vitórias de torneio, metric tournament_win)
...
- E2E agent-browser COMPROVADO (3 campanhas reais):
  ...
  * Campanha 2 (após adiantar cooldown via DB): quartas vs Vera Vento (ROTAÇÃO ✓) VITÓRIA → semi vs Sombra Silente VITÓRIA → final vs Varnil DERROTA (pré-calibração) — HP-carry bloqueou a entrada ferido ("Ferido demais — cure-se antes de subir no ringue") → hospital curou 5.742 Zeni ✓
  * Campanha 3 (pós-calibração): Kuma → Ronin → Varnil FINAL VITÓRIA "+1.800 Zeni • +2.690 XP • +3 💎" → toast 🏆 CAMPEÃO + faixa CAMPEÃO DO GRANDE TORNEIO + palmarés 1 cinturão/7 lutas/🏆 + cooldown reiniciado ✓; DB confirma: títulos=1, roundWins=7, battles 7W/2L, ledger audível (3×300+700+1💎 + 1800+3💎)
  * CONQUISTA COBRADA NA UI: Campeão Mundial 1/1 → claim → +10.000 Zeni +10 💎 aplicados com ledger 'achievement' ✓
...
- E2E COMPROVADO (agent-browser, desktop 1280×800 + mobile 390×844):
  ...
  * QUARTAS tanque (decisão): "R41 ⚖️ DECISÃO — ⚖️ FIM DO TEMPO! 40 rodadas sem nocaute... Rei Taurion terminou com 92% (1181/1290) contra 88% (1020/1162) de Mestre Kuma. Rei Taurion leva a vitória!" → VITÓRIA; Zeni congelado 6.400 → 6.700 (+300) só após Continuar; HP mantido 1184/1290 (decisão NÃO vai pro hospital — barra e DB coerentes)
...
- Mapeamento de TODAS as mecânicas com fonte: combate (MAX_ROUNDS 40, COMBAT{±15% var, soft cap 80%, coefs 0.78/0.74, kiRegen 8%}, Ki básico 10, battleKi 40+ki×4, esquiva 5%+0,5%/pt 3–30% → clamp 2–40%), Ímpeto (6 máx, começa 1, gatilhos 18% HP/metade/Abertura, gastos 1/2/3, Quebra +1 Escala 2 rodas + 30 Ki, Exaustão ×0,85/×1,10), Escala (10 patamares 0→100k, +7%/-12% máx 3, crush ×0,35 a 4+, Abertura ×1,75 3–9%, 3 aberturas = quebra de barreira), fim de luta (KO/DECISÃO %-abs-moeda; hospital 1 HP SÓ em KO; level-up restaura HP pós-luta), atributos (cap 999, curva de custo 20@10 ×1,05/×1,035, teto 50M, treino 3 energia), recursos (energia 5min/pt, HP 12s/pt, maxEnergy 80+2×ki, maxHp 80+15×lvl+5×def, cura 3 Zeni/HP), ações (treino 3⚡, batalha 3⚡, torneio 3⚡, trabalho 6⚡/60min, boss 10⚡/10s), PvP (±5 níveis, ≥20% HP, roubo 8% min 100, derrota paga 5%, atacante ocupado bloqueado/vítima sempre atacável, desafiado entra cheio), torneio (0,82/0,95/1,00×, 300/700/1800 Zeni, 12/18/30% XP, 0/1/3 cristais, cooldown 15min, vida carrega, rejeitado leva METADE do XP, elenco 3+3+2 rotativo), profissões (5, ranks 300→1500 Zeni, 10→25% XP, 3→10% esfera, promoções 1k/3k/9k/30k em 3/4/5/6 conclusões), conquistas (25), quests (3 de 6 diárias, 2 de 5 semanais), esferas (7 → 4 desejos: 8000 Zeni/+3 stats/full/mil XP), world boss (3 chefes 1,2M–1,8M HP/72h, ~60 golpes/hit médio, 15% HP custo, XP dano/200, participação ≥500, 300+1 base, Top50 +200, Top10 +800+1, Top3 +2000+3, #1 +5000+5+título), guildas (5000 Zeni, 60 membros, nível 1+√(xp/500), próximo 500×lvl²), temporadas (30 dias, 10 pts/vitória), raças (5 com mults reais), técnicas (11 + slots 3+S + 7 mestres), estratégias (5), transformações (25 = 5 raças × árvore 5), talentos (3: 2400/3600/5200 Zeni), loja (6 armas, 6 armaduras, 5 acessórios em Zeni; 3 consumíveis + 6 treinos em cristais; elixir 1,6× treino min 4000), contas (3 personagens, convidado→nuvem)
...
- E2E agent-browser (desktop 1280×800 + mobile 390×844):
  ...
  * Spot-check de números na página renderizada: 40 rodadas ✓ · escala 100.000 ✓ · torneio 1.800 ✓ · cura 3 Zeni/HP ✓ · Quebra +1 Escala/2 rodadas ✓ · curva XP 80×nível^1,55 com valores 2.838/8.311 = saída da função real ✓ · Broly 11.000 ✓ · roubo 8% ✓ · ±5 níveis ✓ · decisão 71% → hospital 1 vida ✓
...
Stage Summary:
- Wiki ENTREGUE com transparência total: 23 seções cobrindo TODAS as mecânicas auditadas — Primeiros Passos; Contas e Personagens; Combate Turno a Turno; Ímpeto; Escala de Poder e Scouter; Fim de Luta KO×Decisão; Estratégias; Técnicas; Talentos de Ímpeto; Atributos; Recursos e Economia; Ações/Custos/Ocupação; Raças (Classes); Transformações; Profissões; Campanhas PvE; PvP; Torneio; Ameaça Universal (World Boss); Esferas do Dragão; Guildas e Temporadas; Conquistas e Missões; Loja e Itens
...
- 11 DIVERGÊNCIAS confirmadas e corrigidas no wiki-content.ts (tabela no relatório final): profissão gastava 6⚡ (constante MORTA — v0.9 removeu a cobrança), treino 1,6s (MORTO — treino é instantâneo), "enquanto trabalha você pode lutar" (falso — allowlist bloqueia), boss ~60 golpes (real: 8 golpes ×60), elixir 1,6× preço de treino (fonte duplicada — hoje 75 💎 fixo), Androide "trabalhos custam 15% menos energia" (morto), loja liberada "sempre" (bloqueada em trabalho), atividades treino/batalha com duração (só batalha), card de torneio atribuído ao painel errado
...
- Constantes mortas marcadas com "// LEGADO — não usado, ver wiki-audit v0.9.23" (world.ts, rules.ts, engine.ts); perk do Androide corrigido NO JOGO (races.ts: "Trabalhos rendem +5% de Zeni" — o texto antigo mentia na tela de criação)
- CAMADA 3 (E2E comportamental, agent-browser): 9 verificações comprovadas no jogo rodando — (1) trabalhar NÃO gasta energia (100→100), (2) luta bloqueada durante trabalho (botão EM TURNO + allowlist), (3) treino instantâneo −3⚡ −20 Zeni Força 10→11, (4) batalha PvE −3⚡ com replay, (5) boss −10⚡ e −15% vida por ataque (160→136), (6) hospital custou exatamente 3 Zeni/HP faltante, (7) regeneração de vida observada (+1/12s), (8) +5% Zeni racial na recompensa de trabalho (implícito), (9) wiki renderiza 23 seções + 16 expandíveis + 79 links internos 0 quebrados
...
- Recaída prevenida: constantes mortas marcadas LEGADO; perks da raça Androide corrigidos no jogo; contrato vigia os 2 pontos de recaída conhecidos
- Arquivos alterados: src/lib/wiki/wiki-content.ts (reescrito), src/components/wiki/WikiView.tsx (resumo + details), src/lib/worldboss.ts (exports), src/lib/game/engine.ts (exports + LEGADO), src/lib/game/rules.ts (LEGADO trainMs), src/lib/game/content/world.ts (LEGADO PROFESSION_ENERGY_COST), src/lib/game/content/races.ts (perk Androide) — Criados: tests/wiki-contract.test.ts
...
- Conta QA criada: "Principe QAum" (Namekuseijin, estratégia Especialista em Ki) + kamehameha/taiyoken equipadas via setup direto
...
- B2 XP/Zeni lentos: CAUSA RAIZ em claimAchievement — update otimista soma XP SEM recalcular nível ("363/234 nível 2" possível); demais fluxos sem otimismo (UI presa valor velho durante round-trip lento); servidor grantXp é atômico (nível+carry juntos)
...
- C2 Hospital caro: confirmado — 3 Zeni/HP, sem cura grátis diária; derrota KO (1 HP) + cura total ~231 Zeni no nível 3 vs vitória PvE 68
...
- D2 Guilda inacessível: REPRODUZIDO — "Guildas do universo (0)", fundar custa 5.000 Zeni; sem seed de guilda
...
- AUDITORIA DO ESTADO: 398/398 testes passando (base 382 + 16 novos), lint ✓, tsc ✓, migrações prisma registradas (freeHealDay + Guild.isSystem), guilda do sistema já no banco (Tropa da Tartaruga + Mestre Kame bot nv8), banco pristino
...
- E2E NOVOS (agent-browser, personagem QA convidado nv1-2 Namekuseijin):
  * D2: guildas mostra "Tropa da Tartaruga Nv 1 🐢 pública · Líder Mestre Kame" no nv 1 → "Entrar na guilda" → 1 membro + boas-vindas (d2-guilda-entrada.png)
  ...
  * C2: dano de batalha → "1ª cura do dia GRÁTIS" → cura total com Zeni INALTERADO (572) + "Tratamento de cortesia"; depois chefe mundial (−15% vida) → "cura de hoje: 72 Zeni (a gratuita já foi usada)" → curou cobrando 69 (3×23 HP realmente faltantes — regen devolveu +1 HP; cobra só o que falta) (c2-cura-paga-mesmo-dia.png)
  * B2 CENÁRIO EXATO DO PLAYTEST: nv 1 com 46/80 XP coleta "Primeiro Sangue" (+200 Zeni +50 XP +1💎) → MESMA renderização: 🪙766 💎1 ⭐16/234 Nível 2 (carry 96−80=16) — impossível capturar estado inválido (b2-coleta-atomica-nivel-junto.png)
  * D1: comprou Rogafufuken (648→48 instantâneo, auto-equip slot 1) → batalha usou técnica já na R1 "🔥 TÉCNICA 👊 COMBO QA D2 Teste encadeia o combo com 🐺 Rogafufuken!" (d1-tecnica-usada-batalha-real.png)
  ...
  * Wiki renderizada com todos os valores novos: taxa 200, cooldown 30min, cura grátis, garantia de espetáculo, Tropa da Tartaruga (v0924-wiki-atualizada.png)
...
- LIMPEZA: personagens QA ("QA D2 Teste", "QA B2 Coleta") + contas guest removidos — banco final: 0 contas, 0 humanos, 16 bots (15 + Mestre Kame), 1 guilda sistema, 0 sessões/atividades/dedup
...
Stage Summary:
- 9/9 PROBLEMAS ENTREGUES: A1 timeout+retry+banner (nunca tela morta; servidor é a fonte da verdade) · A2 ±5 por nível + blockReason específico + sparring (densidade garantida) · A3 espaços reais em todos os 9 marcadores · B1 skeleton+timeout 8s+retry nas 5 telas (PanelLoad.tsx) · B2 optimistic.ts — XP+level+carry ATÔMICO generalizado para 14 tipos de ação, probe de 500 deltas aleatórios · C1 torneio 150/400/800 + taxa 200 + cooldown 30min (campeão ~2.156 Zeni/h = 7,7× PvE; era 35×) · C2 primeira cura do dia grátis (freeHealDay, fuso SP, sem cron) · D1 garantia ≥1 técnica/batalha a partir da rodada 3 (100% em 4.800 lutas; típicas preservadas) · D2 Tropa da Tartaruga + Mestre Kame (ensureSystemGuild idempotente, não dissolve, fundação 5.000 intacta)
...
- Walkthrough REAL ao vivo (agent-browser, sessão limpa f5map, 1280×800, cookies zerados): landing → clique "JOGAR GRÁTIS" → AuthGate (modo default "Entrar") → clique "Jogar como convidado" (0 dados pedidos) → criação (nome obrigatório 2–20 chars validado ao vivo com "⚠ Escolha um nome com pelo menos 2 caracteres!"; raça Saiyajin e sexo ♂ pré-selecionados) → personagem "F5 Map Guerreiro" criado → dashboard sem nenhum modal (toasts auto-dismiss: "Salve, guerreiro!" e "Bem-vindo, guerreiro!") → aba "Batalha" → "Lutar!" no Saibaman Verde → replay de 12 rodadas (~20-25s, fundo congelado) → VITÓRIA "+74 Zeni • +53 XP" → "Continuar"
- CLIQUES OBRIGATÓRIOS landing→1ª batalha INICIADA: **5** (JOGAR GRÁTIS · Jogar como convidado · Começar como Saiyajin ♂ · aba Batalha · Lutar!) + digitar nome (ou +1 clique no 🎲). Até a batalha CONCLUÍDA: 6 (Continuar). TELAS distintas: 6 + boot transitório
...
- RESULTADOS por config (win rate % na ordem Saiyajin/Humano/Namekuseijin/Androide/Majin · % DECISÃO total · dispersão máx pp):
  * C1 iniciante: 58,10 / 61,65 / 45,95 / 29,30 / 55,00 · DECISÃO 0,9% · 20,7pp ⚠️ Androide FORA (<35%; IC95 [27,3–31,3])
  ...
  * C6 endgame: 36,30 / 52,25 / 39,75 / 63,40 / 58,30 · 0,0% · 13,7pp ✅ (borderline: Androide a 1,6pp do teto 65%; Saiyajin a 1,3pp do piso 35%)
- ROBUSTEZ (sementes 250–499) reproduz todas as conclusões qualitativas: C1 Androide 25,7% (<35% confirmado nos dois blocos), C2 inteiramente dentro de 35–65%, viés C6 Androide>Saiyajin mantido (56,2% vs 38,1%) — magnitude de extremos oscila entre blocos de semente (ler com IC95, não ponto fixo)
- PADRÃO EMERGENTE (apenas dados): nos cenários sem equipamento a hierarquia INVERTE com a progressão — Androide sobe faixa a faixa (29,3→63,4%) e Saiyajin cai (58,1→36,3%), cruzando entre C2 e C3; Majin nunca fica abaixo de 55% em NENHUMA config; DECISÃO praticamente extinta (máx 0,9% no C1 — KO dominante com MAX_ROUNDS=40); rodadas médias 32,6 (C1) → 17,6 (C5) → 20,1 (C6); HP% fim médio 2,6–9,7%

Stage Summary:
- VEREDITO: a suspeita antiga (Majin 85,9% / Androide 14,8%) NÃO se reproduz no engine atual (HEAD 381909f) — no cenário comparável C2 (mesmo desenho da calibração antiga: nv10, atributos 50, sem itens/técnicas) Majin = 56,10% e Androide = 43,70%; todas as raças dentro do corredor 35–65% em C2, C3, C4, C5 e C6; o ÚNICO ponto fora do corredor ±15pp em todo o levantamento é o Androide do C1 iniciante (29,30%)
...
- Pontos para o agente principal decidir (NADA foi alterado): (1) Androide fraco no early game (C1 29,3% — +5% velocidade, +2% esquiva e +2% ki-chance valem pouco com atributos 15 e sem técnicas); (2) curva invertida Androide↑/Saiyajin↓ com a progressão (esquiva/velocidade/ataques de Ki ganham valor absoluto crescendo; o +8% físico do Saiyajin perde valor relativo); (3) Majin consistentemente 55–58% em TODAS as configs (nunca abaixo do centro); (4) C6 borderline (13,7pp)
...
- F2 ECONOMIA: custos marginais atuais 140/1.614/18.515/103.406 (vs antiga 5.787/134.888/3.14M — curva v0.4 já achatou 7-30×); elixir custa 75💎 (v0.9.2), elixirPrice() é código morto; cruzamento contábil no atributo ~75-104 (💎≈50-200 Zeni) mas limitado: treino teto 96 pontos/dia (energia), elixir casual 0,9 ponto/dia e hardcore 4 pontos/dia ESPALHADOS (renda 💎 4/dia diárias + 3,4/semanais + torneio 30💎/dia teto ENERGIA 9⚡/campanha) → NÃO CONFIRMADA; solução mais conservadora = nenhuma mudança (alternativas descartadas documentadas)
- F3 RACIAL: git mostra races.ts congelado desde 13/09 (v0.4); auditoria externa é PRÉ-calibração; round-robin atual: Majin 56,1/Androide 43,7 no C2 — dados antigos não se reproduzem; ÚNICO gatilho real: Androide C1 29,3% (20,7pp) + inversão de hierarquia com progressão (Androide 29,3→63,4; Saiyajin 58,1→36,3); DECISÃO ≤0,9% (fix MAX_ROUNDS firme)
- F3 AJUSTE: experimentação dirigida (scripts/sim-f3-ajuste.ts, 13 candidatos): dano/defesa uniformes estouram C6 (66-80%); solução Pareto = trocar escala de alto nível por base de baixo: Androide dodgeBonus 0,02→0,045 + speedMult 1,05→1,035 + kiAttackChanceBonus 0,02→0,06 → C1 38,5% (disp. 11,5pp), C2 50,9%, C6 63,9% preservado; perks/wiki/contrato/tests sincronizados (wiki-contract NOVO valida wiki==código e bane texto antigo)
...
Task: CORREÇÃO URGENTE v0.15 — (1) eliminar PERMANENTEMENTE a guilda de sistema "Tropa da Tartaruga" (matar mecanismo + erasure da instância + verificação em duas camadas); (2) corrigir exclusão de personagem SÓ-NUVEM que falhava com "NADA foi apagado" (diagnóstico com evidência + cobertura de todas as origens)

Work Log:
- FORENSE INICIAL: banco dev já tinha 0 guildas (a linha da Tropa morreu no cleanup v0.14), MAS o mecanismo continuava vivo (ensureSystemGuild no GET /api/game/guilds ressuscitaria na próxima visita — exatamente a armadela prevista). Mestre Kame: bot guildless nv 8. AdminActionLog local: 9 entradas do E2E v0.14, NENHUMA tentativa de "Principe Aisurom" → a tentativa falha do dono ocorreu na implantação de PRODUÇÃO (v0.14 já deployed)
...
- E2E REAL (agent-browser, sessão nomeada, 25 screenshots em download/qa-v016-*.png): (1) criação de convidado SEM passo de sexo (2 passos) → dashboard sem nenhum marcador ♂/♀/Masculino (eval false) → 1ª batalha PvE VITÓRIA +67 Zeni; (2) guilda fundada antes do trabalho (QA Gilda Matriz); (3) trabalho iniciado — missionEndsAt ORIGINAL medido 2026-09-16T04:59:16.126Z; (4) DURANTE trabalho: treino NEGADO (client "EM TURNO"+hint + server 409 PLAYER_BUSY_ON_MISSION com mensagem que lista os liberados) · PvE NEGADO (client+server 409) · torneio NEGADO ("Torneio liberado só após o fim do turno de trabalho") · hospital OK (HP 28→145, cura grátis do dia) · Chefe Global OK (dano real, energia −10) · loja OK (−300 Zeni exato) · doação OK (guilda +100, donations=1) · conquista Primeiro Sangue OK (+200 Zeni/+50 XP/+1 cristal) · PvP de sparring OK (batalha completa, derrota com roubo de 5% = −768 exato — processada POR INTEIRO durante o trabalho) — missionEndsAt INALTERADO (04:59:16.126Z) APÓS TODAS as ações; (5) acelerador QA (missionEndsAt→passado, só após a medição de integridade) → coleta por UI: "+300 Zeni, +24 XP" = recompensa INTEGRAL do agricultor rank 1, professions registradas, slot livre; (6) COLETA DURANTE BATALHA: trabalho #2 acelerado → batalha PvE iniciada → claim_mission via fetch DA PÁGINA com [role=dialog] ABERTO (replay rodando) → HTTP 200 OK → batalha VITÓRIA +69 Zeni intacta + coleta +300 intacta + zeni fechando ao centavo (14443+300+69=14812); (7) 3ª batalha → diária "Sangue quente" 3/3; (8) MOBILE 390px (device iPhone 14, nav inferior): trabalho #4 ativo → compra −300 exato · doação −200 exato (donations=2, total 300) · coleta da diária +400/+1 cristal — endsAt 05:08:55.891Z INALTERADO nas três; (9) wiki: callout "TRABALHANDO bloqueia APENAS 3 ações" + tabela ação×estado renderizada + "coletar é sempre possível" + zero "sexo"; (10) dev.log: ZERO erros 500 durante todo o E2E; (11) SQL: matriz anti-órfã 0 pendências, estado consistente
